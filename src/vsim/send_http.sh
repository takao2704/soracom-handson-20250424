...
# 引数の解析
ENDPOINT="https://api.soracom.io/v1/devices/data"
INTERVAL=5
COUNT=1

while getopts "e:i:c:h" opt; do
    case $opt in
        e)
            ENDPOINT="$OPTARG"
            ;;
        ...
        c)
            ...
            ;;
        h)
            echo "使用方法: $0 [-e エンドポイント] [-i 送信間隔(秒)] [-c 送信回数]"
...
send_data() {
    ...
    log_info "HTTP POSTでデータを送信中... ($ENDPOINT)"
